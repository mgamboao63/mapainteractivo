...
searchInput.addEventListener("input",renderResults);document.getElementById("clearSearch").onclick=()=>{searchInput.value="";results.innerHTML="";searchInput.focus()};document.getElementById("backSearch").onclick=()=>{window.speechSynthesis?.cancel();tracking=false;orientationEnabled=false;startTrackingButton.classList.remove("active");toggleOrientationButton.classList.remove("active");startTrackingButton.textContent="Iniciar";toggleOrientationButton.textContent="Orientación";window.removeEventListener("devicemotion",detectStep);window.removeEventListener("deviceorientation",handleOrientation,true);routePanel.hidden=true;welcome.hidden=false;routeSegments=[];currentPath=[];selectedDestination=null;drawPath([]);searchInput.focus()};document.getElementById("previousSegment").onclick=()=>showSegment(segmentIndex-1);document.getElementById("nextSegment").onclick=()=>showSegment(segmentIndex+1);document.querySelectorAll("[data-floor]").forEach(button=>button.onclick=()=>switchFloor(Number(button.dataset.floor)));document.getElementById("centerRoute").onclick=centerOnRoute;document.getElementById("showAll").onclick=showAll;document.getElementById("zoomIn").onclick=()=>{scale=Math.min(3.8,scale+.18);applyTransform()};document.getElementById("zoomOut").onclick=()=>{scale=Math.max(.65,scale-.18);applyTransform()};startTrackingButton.onclick=startTracking;stepBackButton.onclick=()=>moveOneStep(-1);stepForwardButton.onclick=()=>moveOneStep(1);toggleOrientationButton.onclick=toggleOrientation;calibrateOrientationButton.onclick=calibrateOrientation;document.getElementById("repeatAudio").onclick=()=>speak(lastSpeech,true);document.getElementById("toggleAudio").onclick=event=>{audioMuted=!audioMuted;if(audioMuted)window.speechSynthesis?.cancel();event.currentTarget.textContent=audioMuted?"🔊 Activar audio":"🔇 Silenciar"};
...
svg.addEventListener("pointermove",event=>{if(!activePointers.has(event.pointerId))return;const p=pointer(event);activePointers.set(event.pointerId,p);if(activePointers.size>=2&&pinchStart){const points=[...activePointers.values()],distance=pointerDistance(points),center=pointerCenter(points),ratio=distance/(pinchStart.distance||distance||1);scale=Math.max(.65,Math.min(4.8,pinchStart.scale*ratio));panX=pinchStart.panX+(center.x-pinchStart.center.x);panY=pinchStart.panY+(center.y-pinchStart.center.y);applyTransform();return}if(dragging&&activePointers.size===1){panX+=p.x-lastPoint.x;panY+=p.y-lastPoint.y;lastPoint=p;applyTransform()}});
...
svg.addEventListener("wheel",event=>{event.preventDefault();scale=Math.max(.65,Math.min(4.8,scale+(event.deltaY>0?-.12:.12)));applyTransform()},{passive:false});
